...
# Copy MinGW runtime DLLs required at runtime (POSIX threading model needs winpthread)
WINPTHREAD_DLL="$(dirname "$(g++ -print-file-name=libwinpthread-1.dll)")/../bin/libwinpthread-1.dll"
if [ ! -f "$WINPTHREAD_DLL" ]; then
    WINPTHREAD_DLL="$(dirname "$(which g++)")/../bin/libwinpthread-1.dll"
fi
if [ -f "$WINPTHREAD_DLL" ]; then
    cp "$WINPTHREAD_DLL" "$OUTPUT_DIR/libwinpthread-1.dll"
    echo "  Installed libwinpthread-1.dll to: $OUTPUT_DIR/"
else
    echo "  Warning: libwinpthread-1.dll not found" >&2
fi
